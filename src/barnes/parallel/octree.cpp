...
void Octree::computeAcceleration(int idxGlobal, double (*r)[3], double (*a)[3], double g, double theta) {
    ...
    double d = sqrt( (centerMass[0] - r[idxGlobal][0]) * (centerMass[0] - r[idxGlobal][0])
                   + (centerMass[1] - r[idxGlobal][1]) * (centerMass[1] - r[idxGlobal][1])
                   + (centerMass[2] - r[idxGlobal][2]) * (centerMass[2] - r[idxGlobal][2]));

    if (2.*boundary.w/d <= theta) {
        
        double rji[3];
        rji[0] = centerMass[0] - r[idxGlobal][0];
