...
void computeAcceleration(const size_t N, sim::data_type (*rx), sim::data_type (*ry), sim::data_type (*rz),
        sim::data_type (*ax), sim::data_type (*ay), sim::data_type (*az), sim::data_type *m, double& compute_time,double& setup_time) {
	...
	time_point_t setup_start, setup_end;
	time_point_t compute_start, compute_end;
	...
	setup_time += std::chrono::duration< double > (setup_end - setup_start).count();
...
//Computation of Initial Energy
    ...
    computeAcceleration(params.n, rx, ry, rz, ax, ay, az,  m, compute_time, setup_time);
    ...
    for (size_t t = 0; t < Ntimesteps; t++) {
        ...
        comp_start = std::chrono::high_resolution_clock::now();
        computeAcceleration(N, rx, ry, rz, ax, ay, az,  m, compute_time, setup_time);
        comp_end = std::chrono::high_resolution_clock::now();
        comp_time += std::chrono::duration< double >(comp_end - comp_start).count();
