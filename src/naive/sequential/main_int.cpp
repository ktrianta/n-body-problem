 ...
 void computeAcceleration(const size_t N, sim::data_type (*r)[3], sim::data_type (*a)[3], sim::data_type *m) {
	...
	for (size_t j = 0; j < N; j+=4) { /* Get contribution of all j particules on particule i. */
		...
		__m256 sub2  = _mm256_sub_pd( rj2, ri2 );
 		__m256 mult0 = _mm256_mul_pd( sub0, sub0 );
		__m256 mult1 = _mm256_mul_pd( sub1, sub1 );
		__m256 mult2 = _mm256_mul_pd( sub2, sub2 );  
 		__m256 add      = _mm256_add_pd(_mm256_add_pd(_mm256_mul_pd(sub0,sub0), _mm256_mul_pd( sub1, sub1 )),_mm256_mul_pd(sub2,sub2)); 
 		__m256 denom    = _mm256_mul_pd(_mm256_sqrt_pd(add), add);        
		__m256 zeros = _mm256_set1_pd(0);
		__m256 cmp_res = _mm256_cmpeq_epi64(zeros,denom);	
		__m256 masses = _mm256_load_pd(m+j);
 		__m256 a_i = _mm256_div_pd(_mm256_mul_pd(_mm256_set1_pd(-sim::g),masses),denom);
		...
		tt +=   std::chrono::duration < double > ( te - ts).count();
...
//    time_point_t t_start, t_end; 
//    t_start = std::chrono::high_resolution_clock::now();
    computeAcceleration(params.n, r, a, m);
//    t_end = std::chrono::high_resolution_clock::now();
//    double ta = std::chrono::duration< double > (t_end - t_start).count(); 
...
//	 t_end = std::chrono::high_resolution_clock::now();
// 	 ta += std::chrono::duration < double > ( t_end - t_start).count();
