...
//    for(int i = 0; i < size; i++) {
        bool success = false;
        if (!success) success = northWest->insert(index);
        if (!success) success = northEast->insert(index);
        if (!success) success = southWest->insert(index);
        if (!success) success = southEast->insert(index);
        index = -1;
//    }
