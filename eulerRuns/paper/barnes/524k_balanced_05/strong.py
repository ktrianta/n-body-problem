...
for p in NUM_PR:
    sample = np.zeros( (p,NUM_MEAS) )
    for i in range(0,p):
        sample[:][i] = genfromtxt("../524k_parallel_05/"+str(p)+"/plot"+str(i)+".txt",usecols = 0,unpack=True,delimiter=',',skip_header=1,skip_footer=0)
    medians = np.median(sample,axis = 0)
...
ax.plot(NUM_PR[1:],global_median_total[1:],'--o')
ax.plot(NUM_PR[1:],global_median_comm[1:],'--s')
#ax.plot(NUM_PR[1:],global_median_comp[1:],'--o')
#ax.plot(NUM_PR[1:],global_median_gather[1:],'--o')
#ax.plot(NUM_PR[1:],global_median_tree[1:],'--o')
...
ax.legend([r'$\mathrm{Equal \ \# \ of \ particles}$',r'$\mathrm{Equal \ amout \ of  \ work}$'],frameon=False);
ax.set_xticks([2,4,8,16,32,48])
ax.get_xaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
ax.get_yaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())

textstr = r'$\theta=0.5$,  $N=500k \ \mathrm{particles}$'
...
plt.ylim(0.001, 70)
#plt.show()
plt.savefig("500k025Comparison.eps",transparent=True)
